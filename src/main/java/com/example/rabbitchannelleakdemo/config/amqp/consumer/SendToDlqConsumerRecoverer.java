...
@Slf4j
public class SendToDlqConsumerRecoverer implements RabbitmqConsumerRecoverer {
    ...
    public boolean recoverMessage(Message<?> message, Throwable throwable) {
        ...
        try {
            var result = producer.publish("DLX", queueName, MessageBuilder.fromMessage(message)
                    ...
                    .setHeader("x-exception-message", reduceStringTo(exceptionMessage, 1000))
                    .setHeader("x-exception-stacktrace", reduceStringTo(stackTrace.toString(), 10_000))
                    .build());
    ...
    private String reduceStringTo(String value, int maxLength) {
        if (value == null) {
            return null;
        }
