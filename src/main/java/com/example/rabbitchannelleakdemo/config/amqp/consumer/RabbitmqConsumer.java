...
import static io.micrometer.observation.Observation.tryScoped;
import static io.micrometer.observation.Observations.getGlobalRegistry;
...
public final class RabbitmqConsumer<T> implements Consumer<Message<T>> {
    ...
    // sendToDlq | sendToExceptionHandler | ...
    private final RabbitmqConsumerRecoverer recoverer;
    ...
    public void accept(Message<T> message) {
        ...
        try {
            ...
            Observation currentObservation = getGlobalRegistry().getCurrentObservation();

            executorService.execute(() -> tryScoped(currentObservation, () -> {
    ...
    // Do not rename this method (It is going to be instrumented in Dynatrace by package, class and method name)
    private void processWithDelegate(Message<T> message) {
